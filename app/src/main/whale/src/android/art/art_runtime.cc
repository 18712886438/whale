...
std::set<void*> hookedMethods = std::set<void*>();
void* (*PreGetOatQuickMethodHeader)(void *thiz, void*, uintptr_t) = nullptr;

void* NewGetOatQuickMethodHeader(void *thiz, void* artMethod, uintptr_t pc) {
...
void ArtRuntime::addHookedMethod(JNIEnv *env, jclass type, jobject hookMethod) {
    hookedMethods.insert(env->FromReflectedMethod(hookMethod));
}

bool ArtRuntime::HookGetOatQuickMethodHeader() {

    static Singleton<bool> enforced([&](bool *result) {
        *result = HookGetOatQuickMethodHeaderImpl();
    });
    return enforced.Get();
}

ALWAYS_INLINE bool ArtRuntime::HookGetOatQuickMethodHeaderImpl() {
    ...
    if (_handle) {
        WInlineHookFunction(_handle, reinterpret_cast<void*>(NewGetOatQuickMethodHeader),
                            reinterpret_cast<void **>(&PreGetOatQuickMethodHeader));
    }
